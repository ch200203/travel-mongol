  ...
  return <section>
    ...
    <div className="timeline">{[1, 2, 3, 4, 5, 6].map((day) => {
      ...
      return <section className="day-group" key={day}>
        ...
        <div className="day-items">{items.length === 0 ? <div className="empty small">등록된 일정이 없어요.</div> : items.map((item) => <article className={`schedule-item${item.status === 'cancelled' ? ' cancelled' : ''}`} key={item.id}>
